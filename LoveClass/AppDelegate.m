...
- (void)setRootVC {
    ...
    if (![[NSUserDefaults standardUserDefaults] boolForKey:@"NotFirstLaunch"]) {
        ...
        MainVC *vc = [[UIStoryboard storyboardWithName:@"Main" bundle:[NSBundle mainBundle]] instantiateViewControllerWithIdentifier:@"MainVC"];
        self.window.rootViewController = vc;
        
        if ([User loadUser].isLogin && !isEmpty([User loadUser].account) && !isEmpty([User loadUser].pwd)) {
            [self requestForLoginWithName:[User loadUser].account pwd:[User loadUser].pwd];
        }
    }
}
