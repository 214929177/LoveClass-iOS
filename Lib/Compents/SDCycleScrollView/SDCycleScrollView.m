...
- (void)automaticScroll
{
    ...
    if (targetIndex < _totalItemsCount/100 - 1) {
...
//        [_mainView scrollToItemAtIndexPath:[NSIndexPath indexPathForItem:_totalItemsCount * 0.5 inSection:0] atScrollPosition:UICollectionViewScrollPositionNone animated:NO];
    ...
    CGFloat x = self.sd_width - size.width - 10;
    if (self.pageControlAliment == SDCycleScrollViewPageContolAlimentRight) {
        x = self.mainView.sd_width - size.width - 10;
    }
    CGFloat y = self.mainView.sd_height - size.height - 10;
    _pageControl.frame = CGRectMake(x, y + 5, size.width, size.height);
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [cell.imageView sd_setImageWithURL:self.imagesGroup[itemIndex] placeholderImage:[UIImage imageNamed:@"alpha"] completed:^(UIImage *img, NSError *error, SDImageCacheType cacheType, NSURL *imageURL){
        cell.imageView.image = img;//[img stretchableImageWithLeftCapWidth:160 topCapHeight:10];
    }];
