...
@interface ALMoviePlayerController () {
    BOOL isMovieFullScreen;//used to manipulate default fullscreen property
}
...
- (id)initWithFrame:(CGRect)frame {
    if ( (self = [super init]) ) {
        ...
        if (!_movieBackgroundView) {
            ...
            UIImageView *img = [[UIImageView alloc]initWithFrame:_movieBackgroundView.bounds];
            img.image = [UIImage imageNamed:@"player_bg.jpg" ];
            img.tag = 10001;
            [_movieBackgroundView addSubview:img];
        }
    }
    return self;
}
...
- (void)setFullscreen:(BOOL)fullscreen animated:(BOOL)animated {
    ...
    if (fullscreen) {
        ...
        [UIView animateWithDuration:animated ? fullscreenAnimationDuration : 0.0 delay:0.0 options:UIViewAnimationOptionCurveLinear animations:^{
            ...
            if ([self.delegate respondsToSelector:@selector(moviePlayerWillMoveToWindow)]) {
                [self.delegate moviePlayerWillMoveToWindow];
            }
...
- (void)rotateMoviePlayerForOrientation:(UIInterfaceOrientation)orientation animated:(BOOL)animated completion:(void (^)(void))completion {
    CGFloat angle;
    CGSize windowSize = [UIApplication sizeInOrientation:UIInterfaceOrientationLandscapeRight];
...
- (void)play {
    ...
    if ([self.delegate respondsToSelector:@selector(moviePlayerWillBeginPlay)]) {
        [self.delegate performSelector:@selector(moviePlayerWillBeginPlay)];
    }
