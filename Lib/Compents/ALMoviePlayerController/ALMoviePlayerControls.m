...
- (void)setup {
    ...
    _titleLab.text = _movieTitle;
    
    _durationSlider = [[UISlider alloc] init];
    _durationSlider.value = 0.f;
    [_durationSlider setThumbImage:[UIImage imageNamed:@"home_100"] forState:UIControlStateNormal];
    ...
    _timeRemainingLabel.layer.shadowColor = [UIColor grayColor].CGColor;
    ...
    [_stopBtn addTarget:self action:@selector(continuePlay) forControlEvents:UIControlEventTouchUpInside];
    _stopBtn.hidden = YES;
    [_stopBtn setImage:[UIImage imageNamed:@"vedio_4"] forState:UIControlStateNormal];
    ...
    if (_style == ALMoviePlayerControlsStyleFullscreen || (_style == ALMoviePlayerControlsStyleDefault && _moviePlayer.isFullscreen)) {
        ...
        _fullscreenButton.frame = CGRectMake(0, 0, 44, 44);
        _fullscreenButton.imageView.contentMode = UIViewContentModeCenter;
        [_fullscreenButton setImage:[UIImage imageNamed:@"back"] forState:UIControlStateNormal];
        [_fullscreenButton addTarget:self action:@selector(fullscreenPressed:) forControlEvents:UIControlEventTouchUpInside];
        [_topBar addSubview:_fullscreenButton];
        ...
        _scaleButton.frame = CGRectMake(0, 0, 44, 44);
        _scaleButton.imageView.contentMode = UIViewContentModeCenter;
        [_scaleButton setImage:[UIImage imageNamed:@"movieEndFullscreen.png"] forState:UIControlStateNormal];
        [_scaleButton addTarget:self action:@selector(scaleButtonAction:) forControlEvents:UIControlEventTouchUpInside];
        [_bottomBar addSubview:_scaleButton];
        
        _shareBtn = [[UIButton alloc]init];
        _shareBtn.titleLabel.font = [UIFont systemFontOfSize:13];
        _shareBtn.frame = CGRectMake(MainScreen_Height - 60, 0, 44, 44);
        _shareBtn.imageView.contentMode = UIViewContentModeCenter;
        [_shareBtn setImage:[UIImage imageNamed:@"vedio_20"] forState:UIControlStateNormal];
        [_shareBtn addTarget:self action:@selector(shareAction:) forControlEvents:UIControlEventTouchUpInside];
        [_topBar addSubview:_shareBtn];
        
        
        /*
        _volumeView = [[MPVolumeView alloc] init];
        [_volumeView setVolumeThumbImage:getImgWithName(@"home_100") forState:UIControlStateNormal];
        ...
        _stopBtn.frame = CGRectMake((self.moviePlayer.view.frame.size.width - 60)/2, (self.moviePlayer.view.frame.size.height - 60)/2, 60, 60);
    }
...
- (void)layoutSubviews {
    ...
    CGFloat paddingBetweenLabelsAndSlider = 20.f;
    ...
    if (self.style == ALMoviePlayerControlsStyleFullscreen || (self.style == ALMoviePlayerControlsStyleDefault && self.moviePlayer.isFullscreen)) {
        ...
        self.shareBtn.frame = CGRectMake(self.topBar.frame.size.width - paddingFromBezel - fullscreenWidth, self.barHeight/2 - fullscreenHeight/2, fullscreenWidth, fullscreenHeight);
        self.titleLab.frame = CGRectMake(paddingFromBezel + fullscreenWidth + paddingFromBezel, self.barHeight/2 - fullscreenHeight/2, self.topBar.frame.size.width - (paddingFromBezel + fullscreenWidth+paddingFromBezel)*2, fullscreenHeight);
        ...
        self.scaleButton.frame = CGRectMake(self.bottomBar.frame.size.width - paddingFromBezel - scaleWidth, self.barHeight/2 - scaleHeight/2, scaleWidth, scaleHeight);
        ...
        self.timeElapsedLabel.frame = CGRectMake(self.timeRemainingLabel.frame.origin.x - labelWidth, 0, labelWidth, self.barHeight);
        /*
        ...
        _stopBtn.center = self.center;
        
        //duration slider
        CGFloat startX = self.playPauseButton.frame.origin.x + self.playPauseButton.frame.size.width;
        CGFloat endX = self.timeElapsedLabel.frame.origin.x;
        CGFloat silderWidth = (endX - paddingBetweenLabelsAndSlider) - (startX + paddingBetweenLabelsAndSlider);
        self.durationSlider.frame = CGRectMake(startX + paddingBetweenLabelsAndSlider, self.barHeight/2 - sliderHeight/2, silderWidth, sliderHeight);
    }
        ...
        // top bar
        self.topBar.frame = CGRectMake(0, 0, self.frame.size.width, self.barHeight);
        self.titleLab.frame = self.topBar.bounds;
        ...
        //duration slider
        CGFloat startX = self.playPauseButton.frame.origin.x + self.playPauseButton.frame.size.width;
        CGFloat endX = self.fullscreenButton.frame.origin.x;
        CGFloat silderWidth = (endX - paddingBetweenLabelsAndSlider) - (startX + paddingBetweenLabelsAndSlider);
        self.durationSlider.frame = CGRectMake(startX + paddingBetweenLabelsAndSlider, self.barHeight/2 - sliderHeight/2, silderWidth, sliderHeight);
    }
